...
import kotlinx.android.synthetic.main.base_view_float_control.view.*
...
class FloatControlView @JvmOverloads constructor(
    ...
    // 边界属性, 单位是pixel
    private var mBlankLeft: Int = dp2px(BLANK_LEFT_DEFAULT.toFloat())
    private var mBlankRight = dp2px(BLANK_RIGHT_DEFAULT.toFloat())
    private var mBlankTop = dp2px(BLANK_TOP_DEFAULT.toFloat())
    private var mBlankBottom = dp2px(BLANK_BOTTOM_DEFAULT.toFloat())

    private val mScreenWidthInPixel = UIUtils.getScreenWidth(context)
    private val mScreenHeightInPixel = UIUtils.getScreenHeight(context)
    ...
    init {
        LayoutInflater.from(context).inflate(R.layout.base_view_float_control, this)
